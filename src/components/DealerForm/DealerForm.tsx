...
export function DealerForm({ data, onChange, onValidate }: DealerFormProps) {
  ...
  useEffect(() => {
    const selectedCity = cityAndDealers.find(
      (city) => city.Codigo === formData.city
    );
    const dealerOptions =
      selectedCity?.Vitrinas.map((vitrina) => ({
    ...
    <>
      ...
      <Grid justifyContent="center" textAlign="center">
        <Text
          ...
          textAlign="center"
        >
          *Al darle “Enviar” aceptas el tratamiento de datos y términos y
          condiciones.
        </Text>
        <Text
          marginTop="1.25rem"
          ...
          textAlign="center"
        >
          Si quieres revisar esta información ingresa a:
          www.toyota.com.co/politicas-de-privacidad/
        </Text>
      </Grid>
    </>
  );
